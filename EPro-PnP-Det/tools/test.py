...
from epropnp_det.apis import single_gpu_test
...
def main():
    ...
    assert args.out or args.eval or args.format_only or args.show \
        or args.show_dir or args.timer, \
        ('Please specify at least one operation (save/eval/format/show the '
         'results / save the results) with the argument "--out", "--eval"'
         ', "--format-only", "--show", "--show-dir" or "--timer"')
    ...
    if hasattr(model, 'CLS_ORIENTATION'):
        if 'CLS_ORIENTATION' in checkpoint['meta']:
            ...
            model.CLS_ORIENTATION = getattr(dataset, 'CLS_ORIENTATION', None)
