...
def rotate_iou_kernel_eval(
    ...
    if tx < row_size:
        for i in range(col_size):
            offset = row_start * threads_per_block * k \
                     + col_start * threads_per_block + tx * k + i
...
@cuda.jit(fastmath=False)
def rotate_iou_kernel_eval_aligned(
        n, dev_boxes, dev_qboxes, dev_iou, criterion=-1):
    threads_per_block = cuda.blockDim.x
    bx = cuda.blockIdx.x
    tx = cuda.threadIdx.x

    block_size = min(n - bx * threads_per_block, threads_per_block)
    dev_box_idx = threads_per_block * bx + tx

    if tx < block_size:
        dev_iou[dev_box_idx] = devRotateIoUEval(
            dev_boxes[dev_box_idx * 5:dev_box_idx * 5 + 5],
            dev_qboxes[dev_box_idx * 5:dev_box_idx * 5 + 5], criterion)
