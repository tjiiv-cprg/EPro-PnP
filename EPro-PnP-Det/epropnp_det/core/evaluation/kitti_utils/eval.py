...
def get_mAP(prec, criteria='R11'):
    assert criteria in ['R11', 'R40']
    sums = 0
    if criteria == 'R11':
...
def do_eval(gt_annos,
    ...
    mAP_bbox = get_mAP(ret['precision'], criteria=criteria)
    mAP_aos = None
    ...
    if 'bev' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1,
                         min_overlaps)
        mAP_bev = get_mAP(ret['precision'], criteria=criteria)

    mAP_3d = None
    if '3d' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 2,
                         min_overlaps)
        mAP_3d = get_mAP(ret['precision'], criteria=criteria)
    return mAP_bbox, mAP_bev, mAP_3d, mAP_aos


def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
                       compute_aos, criteria='R11'):
...
def kitti_eval_coco_style(gt_annos, dt_annos, current_classes, criteria='R11'):
