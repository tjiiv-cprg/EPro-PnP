...
def set_logger_dir(dirname, action=None):
    ...
    if dir_nonempty(dirname):
        if not action:
            _logger.warn("""Log directory {} exists! Use 'd' to delete it. """.format(dirname))
            _logger.warn("""If you're resuming from a previous run, you can choose to keep it. Press any other key to exit. """)
        while not action:
